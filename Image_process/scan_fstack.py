...
BASE_DIRECTORY = '/mnt/data/raw_images'
BASE_DEST_DIRECTORY = '/mnt/data/local_processed_data'
...
def stack_cycle(in_directory,out_directory,cycle):
    ...
    for chn in CHANNELS:
        if glob(os.path.join(in_directory,cycle,f'*-{chn}-*')):
            chn_info[chn] = imlist_to_df([f for f in os.listdir(os.path.join(in_directory,cycle)) if chn in f and not f.startswith('.')])
