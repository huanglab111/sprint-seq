function fstack_write(imlist,output_name,varargin)
% Focus stacking.
%
% SINTAX:
%   im = fstack(imlist)
...
%tilesize=2303;
%xstart=round((2048-tilesize)/2);
%ystart=xstart;

%Parse inputs:
opts = parseInputs(imlist, varargin{:});
%opts.size(1)=tilesize+1;
%opts.size(2)=tilesize+1;
M = opts.size(1);
N = opts.size(2);
P = opts.size(3);
...
for p = 1:P
    im = imread(imlist{p});
    %im=im(xstart:xstart+tilesize,ystart:ystart+tilesize,:);
    if opts.RGB
        imagesR(:,:,p) = im(:,:,1);
        imagesG(:,:,p) = im(:,:,2);
        imagesB(:,:,p) = im(:,:,3);
...
if opts.RGB
    im(:,:,1) = uint16(sum((imagesR.*fm), 3)./fmn);
    im(:,:,2) = uint16(sum((imagesG.*fm), 3)./fmn);
    im(:,:,3) = uint16(sum((imagesB.*fm), 3)./fmn);
else
    im = uint16(sum((imagesG.*fm), 3)./fmn);
end
imwrite(im,output_name);
