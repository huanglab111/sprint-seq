...
def get_intensity_df(stc_directory,coordinates,cyc_num=SEQ_CYCLE,tophat=True):
...
def get_seq_df(intensity_df,thresholds,cyc_num=SEQ_CYCLE):
    ...
    base_bool_map = {(True,True):'A',(True,False):'T',(False,True):'C',(False,False):'G'}
    base_df = pd.DataFrame({'Y':coordinates[:,0],'X':coordinates[:,1]})
    for cyc in tqdm(range(1,1+cyc_num),desc='Base calling'):
        base_df[f'cyc_{cyc}'] = bool_df.apply(lambda x: get_base(x[f'cyc_{cyc}_cy3'],x[f'cyc_{cyc}_cy5']),axis=1)
    seq_df = pd.DataFrame({'Y':coordinates[:,0],'X':coordinates[:,1]})
    seq_df['Sequence'] = base_df[[f'cyc_{i}' for i in range(1,1+cyc_num)]].agg(''.join,axis=1)
    return seq_df
